...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return (
    <label className="block text-sm font-medium text-[#0f0f0f] mb-1.5">{children}</label>
  );
}

function FieldHint({ children }: { children: React.ReactNode }) {
  return <p className="mt-1 text-xs text-[#606060]">{children}</p>;
}
  ...
  return (
    <div className="min-h-screen bg-[#f9f9f9]">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-[#e5e5e5] h-14 flex items-center px-4 gap-4">
        <a href="/" className="p-2 hover:bg-[#f2f2f2] rounded-full transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base font-bold text-[#0f0f0f]">設定</h1>
        </div>
      </header>
        ...
        <SectionCard title="📌 預設篩選條件">
          <div>
            ...
            <select
              ...
              className="w-full border border-[#d3d3d3] rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-[#1a73e8] cursor-pointer"
            >
              {REGIONS.map((r) => (
        ...
        <SectionCard title="📨 Telegram 推播通知">
          <div>
            ...
            <input
              ...
              className="w-full border border-[#d3d3d3] rounded-lg px-3 py-2.5 text-sm bg-white focus:outline-none focus:border-[#1a73e8] placeholder-[#909090]"
            />
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-[#0f0f0f]">新影片自動推播</p>
              <p className="text-xs text-[#606060] mt-0.5">Cron Job 偵測到新進熱門時自動發送通知</p>
            </div>
            <button
              onClick={() => setNotifyOnNewVideos((v) => !v)}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#ff0000] focus:ring-offset-2 ${notifyOnNewVideos ? "bg-[#ff0000]" : "bg-[#909090]"}`}
              aria-label="切換自動推播"
            >
